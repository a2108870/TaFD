...
@torch.no_grad()
def build_gpgd_bases(trainloader,
                    n_classes: int,
                    rank: int = 128,
...
def save_gpgd_bases(path: str, bases: Dict[int, torch.Tensor]) -> None:
    cpu_bases = {k: v.cpu() for k, v in bases.items()}
    torch.save(cpu_bases, path)


def load_gpgd_bases(path: str, device: torch.device) -> Dict[int, torch.Tensor]:
...
def gpgd_attack(input, y, model, device, bases_dict,
                 steps: int = 10,
                 epsilon: float = 2.0,
    ...
    """
    PCA-based on-manifold attack used for GPGD evaluation.
    ...
    A, q = _stack_class_gpgd_bases(bases_dict, labels, device)
