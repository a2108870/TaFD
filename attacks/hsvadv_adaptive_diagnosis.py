...
def apply_hsv_curves(img, param_h, param_s, param_v, steps):
...
def hsvadv_attack(input, y, target_threat_domain_indices, model, device, lr=1, max_iterations=10, steps=64, bound=16, num_classes=10,
    ...
    for _ in range(max_iterations):
        X_adv = apply_hsv_curves(X_ori, Paras_h, Paras_s, Paras_v, steps)
        outputs = model(X_adv)
