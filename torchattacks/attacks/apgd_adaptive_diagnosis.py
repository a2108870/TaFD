...
class AdaptiveDiagnosisAPGD(Attack):
    def __init__(self, model, norm='Linf', eps=8 / 255, steps=10, n_restarts=1, seed=0,
                 loss='ce', eot_iter=1, rho=.75, verbose=False, diagnosis_loss_scale=1.0):
        super().__init__('AdaptiveDiagnosisAPGD', model)
        self.eps = eps
        self.steps = steps
        self.norm = norm
    ...
    def forward(self, images, labels, target_threat_domain_indices):
        ...
        _, adv_images = self.perturb(images, labels, target_threat_domain_indices, best_loss=True)
        return adv_images
    ...
    def _forward_joint(self, inputs, labels, target_threat_domain_indices):
        if self._normalization_applied:
            inputs = self.normalize(inputs)
        outputs = self.model(inputs)
        cls_logits, diagnosis_logits = unpack_model_outputs(outputs)
        ...
        if diagnosis_logits is not None:
            diagnosis_loss = diagnosis_margin_loss_maximize(diagnosis_logits, target_threat_domain_indices, reduction='none')
            weight = balanced_diagnosis_weight(loss_cls, diagnosis_loss, self.diagnosis_loss_scale)
            loss_indiv = loss_cls + weight * diagnosis_loss
        else:
            loss_indiv = loss_cls

        return cls_logits, loss_indiv

    def attack_single_run(self, x_in, y_in, target_domain_in):
        x = x_in.clone() if len(x_in.shape) == 4 else x_in.clone().unsqueeze(0)
        y = y_in.clone() if len(y_in.shape) == 1 else y_in.clone().unsqueeze(0)
        target_threat_domain_indices = target_domain_in.clone() if len(target_domain_in.shape) == 1 else target_domain_in.clone().unsqueeze(0)
        ...
        for _ in range(self.eot_iter):
            with torch.enable_grad():
                cls_logits, loss_indiv = self._forward_joint(x_adv, y, target_threat_domain_indices)
                loss = loss_indiv.sum()
    ...
    def perturb(self, x_in, y_in, target_domain_in, best_loss=False, cheap=True):
        ...
        if not best_loss:
            ...
            for counter in range(self.n_restarts):
                ...
                if ind_to_fool.numel() != 0:
                    ...
                    tg_to_fool = target_threat_domain_indices[ind_to_fool].clone()
        ...
        for counter in range(self.n_restarts):
            best_curr, _, loss_curr, _ = self.attack_single_run(x, y, target_threat_domain_indices)
