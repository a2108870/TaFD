...
INF = float("inf")
def transform_normlisa(input):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    transform = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    # inputs = input.permute(0,2,3,1)
    img_batch = torch.stack([transform(inputa) for inputa in input])
    # input = img_batch.to(device=device, dtype=torch.float32)
    return img_batch
...
def carlini_wagner_l2(
    ...
    if y is None:
        ...
        pred = model_fn(transform_normlisa(x))
    ...
    for outer_step in range(binary_search_steps):
        ...
        for i in range(max_iterations):
            ...
            logits = model_fn(transform_normlisa(new_x))
...
#
#     # targeted
#     new_x = carlini_wagner_l2(model_fn, x=x, lr=0.01, y=y, confidence=40, binary_search_steps=3, max_iterations=100)
