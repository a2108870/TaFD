...
Unified encoder supporting ResNet and MobileViT backbones via create_encoder(backbone=...).

Usage:
  from models.encoder import create_encoder
  model = create_encoder(backbone='resnet', num_classes=100, num_domains=6, dataset='CIFAR100')
  model = create_encoder(backbone='mobilevit', num_classes=100, num_domains=6, dataset='CIFAR100')

Architecture:
  • ResNetEncoder: ResNet-34 backbone + FCConv (stem + first block conv1 per stage)
  • MobileViTEncoder: MobileViT backbone + FCConv (stem + selected MV2Block depthwise)
  • Both support BPDA surrogate gradient (set_bpda method)
"""
...
class InputNormalize(nn.Module):
    """Input normalization layer with device-consistent buffers."""
...
#  Extracts 15-dim features: 9 energy shares + 3 chroma angles + 3 chroma dominance
# ──────────────────────────────────────────────────────────────────────────
class SpectralFeatureExtractor:
    """
    Spectral feature extractor for threat-domain classification.

    Extracted features:
    - 9 energy shares (YCbCr x low/mid/high frequency)
    - 3 chroma angles
    - 3 chroma dominance values (Cr-Cb difference per band)
    ...
    def features_from_images(self, img_pixel):
        """
        Extract spectral features from images.

        Args:
            img_pixel: [B, 3, H, W] input images

        Returns:
            features: [B, 15] spectral feature vectors
        """
        ...
        # Base features: 9 energy shares + 3 chroma angles
...
class ThreatDomainClassifier(nn.Module):
    """Lightweight CNN classifier for threat-domain prediction."""
...
class ThreatDomainDiagnosis:
    ...
    def __init__(self, num_sources=6, num_threat_domains=3, feature_ids=None, source_names=None):
        ...
        # Support dynamic attack names
        if source_names is not None:
            self.source_names = source_names
        else:
            self.source_names = ['PGD', 'ACE', 'HSVAdv', 'ReColorAdv', 'ALA', 'RetouchUAA']
    ...
    def compute_spectral_features(self, img_pixel_batch, domain_ids=None):
    ...
    def update_features(self, img_pixel_batch, domain_ids):
        with torch.no_grad():
            ...
            feats = self.compute_spectral_features(img_pixel_batch, domain_ids)
            if feats.numel() == 0:
                return False
    ...
    def load_state_dict(self, state_dict, device=None):
        ...
        # Restore feature prototypes to maintain EMA continuity
...
# ──────────────────────────────────────────────────────────────────────────
#  Basic building blocks (FCConv used at selected positions only)
# ──────────────────────────────────────────────────────────────────────────
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, downsample=None,
        ...
        # conv2: always standard convolution
...
class ResNetEncoder(nn.Module):
    def __init__(self, num_sources=6, num_domains=3, fd_num_experts=3,
        ...
        # Input normalization (auto-selected by dataset)
        if dataset in DATASET_STATS:
            stats = DATASET_STATS[dataset]
        else:
            stats = DATASET_STATS['CIFAR100']  # fallback
        self.input_normalize = InputNormalize(mean=stats['mean'], std=stats['std'])

        # Align expert count with domain count
        ...
        # BPDA surrogate gradient switch (disabled by default)
        self.use_bpda = False
        ...
        if source_names is None:
            source_names = ['PGD', 'ACE', 'GPGD', 'StAdv'] if num_sources == 4 else ['PGD', 'ACE', 'HSVAdv', 'ReColorAdv',
                                                                                    'ALA', 'RetouchUAA']

        # Domain diagnosis module (cluster count = num_domains)
    ...
    def set_bpda(self, enabled: bool = True):
        """Enable/disable BPDA surrogate gradient for adversarial attacks."""
    ...
    def count_frequency_convolutions(self):
        """Count FCConv vs standard convolutions (topology-level)."""
        stages = [3, 4, 6, 3]
        fdconv_count = 1 + len(stages)    # initial(1) + 4 stage first-block conv1
        standard_count = 0

        for n in stages:

            standard_count += (2 * n - 1)
        # Add 3 downsample 1x1 convolutions
        standard_count += 3
        total = fdconv_count + standard_count
        print("Convolution statistics:")
        print(f"  FCConv count: {fdconv_count}")
        print(f"  Standard conv count: {standard_count}")
        print(f"  Total convolutions: {total}")
        print(f"  FCConv ratio: {fdconv_count / total:.1%}")
        return {'fdconv_count': fdconv_count, 'standard_count': standard_count,
                'total_count': total, 'fdconv_ratio': fdconv_count / total}



    def extract_spectral_features(self, img_pixel, source_ids):

        return None, self.threat_domain_diagnosis.update_features(img_pixel, source_ids)

    def forward(self, img, domain_ids=None, skip_normalize=False):
        ...
        # Routing: use ground-truth mapping during training, predicted domains otherwise
        if domain_ids is not None:
            domain_assignments = self.get_domain_labels(domain_ids)
        else:
            domain_assignments = predicted_domains

        # Soft router weights for BPDA surrogate gradient (forward unchanged)
        router_weights = None
        if self.use_bpda:
            router_weights = F.softmax(domain_logits, dim=1)  # [B, D]
    ...
    # Save/load (includes domain diagnosis state)
...
class MobileViTEncoder(nn.Module):
    def __init__(self, size=32, num_classes=10,
        ...
        # Attack names aligned with ResNetEncoder
        if source_names is None:
            source_names = ['PGD', 'ACE', 'GPGD', 'StAdv'] if num_sources == 4 else ['PGD', 'ACE', 'HSVAdv', 'ReColorAdv', 'ALA', 'RetouchUAA']
        self.threat_domain_diagnosis = ThreatDomainDiagnosis(
            num_sources=num_sources,
            num_threat_domains=self.num_threat_domains,
            source_names=source_names
    ...
    def count_frequency_convolutions(self):
        ...
        for name, m in self.named_modules():
            if isinstance(m, FCConv):
                ...
                if ('dedicated_experts' in name) or ('residual_conv' in name):
                    continue
                conv2d_count += 1
        total = fdconv_count + conv2d_count
        print("Convolution statistics (excluding FCConv internal expert/residual):")
        print(f"  FCConv count: {fdconv_count}")
        print(f"  Standard Conv2d count: {conv2d_count}")
        print(f"  Total convolutions: {total}")
        print(f"  FCConv ratio: {fdconv_count / total:.1%}" if total > 0 else "  FCConv ratio: N/A")
    ...
    def forward(self, img, domain_ids=None, skip_normalize=False):
        ...
        # Routing: cluster mapping for training, predicted domains for inference
        if domain_ids is not None:
            domain_assignments = self.get_domain_labels(domain_ids)
        else:
            domain_assignments = predicted_domains
        ...
        # Return expert_freqs from stem FCConv (consistent with ResNetEncoder)
...
def create_encoder(backbone='resnet', dataset=None, **kwargs):
    """
    Create an Encoder instance for the specified backbone.

    Args:
        backbone: 'resnet' or 'mobilevit'
        dataset: dataset name for selecting normalization statistics
