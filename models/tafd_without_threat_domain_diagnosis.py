# -*- coding: utf-8 -*-
"""TaFD ablation without threat-domain diagnosis for ResNet-34 and MobileViT."""
...
from models.frequency_conditional_convolution_without_threat_domain_diagnosis import FrequencyConditionalConvolution
...
class SpectralFeatureExtractor:
    """Extract the 15-dimensional spectral descriptor used by the ablation model."""
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, threat_domain_indices=None, dispatch_weights=None, bpda=False, soft_forward=False):
        identity = x
        if self.conv1_is_fd:
...
class TaFDResNet34(nn.Module):
    def __init__(self, num_attack_sources=7, num_threat_domains=2, num_frequency_experts=2,
        ...
        if _imagenet_like:
            self.initial_conv = nn.Sequential(
                FrequencyConditionalConvolution(3, 64, kernel_size=7, stride=2, padding=3, bias=False,
                       num_threat_domains=self.num_threat_domains,
    ...
    def set_forced_threat_domain_index(self, threat_domain_index=None):
        self.forced_threat_domain_index = None if threat_domain_index is None else int(threat_domain_index)

    def set_uniform_threat_domain_mixture(self, enabled=False):
        self.use_uniform_threat_domain_mixture = bool(enabled)

    def count_frequency_convolutions(self):
        """Count FC-Conv and standard convolution modules."""
        stages = [3, 4, 6, 3]
    ...
    def forward(self, img, attack_source_ids=None, skip_normalize=False,
                forced_threat_domain_indices=None):
        ...
        if forced_threat_domain_indices is None:
            forced_threat_domain_index = getattr(self, "forced_threat_domain_index", None)
            if forced_threat_domain_index is not None:
                forced_threat_domain_indices = torch.full(
                    (img.size(0),), int(forced_threat_domain_index),
                    device=img.device, dtype=torch.long
                )

        use_uniform_threat_domain_mixture = bool(getattr(self, "use_uniform_threat_domain_mixture", False))

        if use_uniform_threat_domain_mixture:
            threat_domain_indices = torch.zeros(
                img.size(0), device=img.device, dtype=torch.long
            )
        elif forced_threat_domain_indices is not None:
            threat_domain_indices = forced_threat_domain_indices.to(
                device=img.device, dtype=torch.long
            ).clamp(0, self.num_threat_domains - 1)
        elif attack_source_ids is not None:
        ...
        soft_forward = False
        if use_uniform_threat_domain_mixture:
            dispatch_weights = diagnosis_logits.new_full(
                (img.size(0), self.num_threat_domains),
                1.0 / float(self.num_threat_domains)
            )
            soft_forward = True
        elif self.use_bpda and forced_threat_domain_indices is None:
            dispatch_weights = F.softmax(diagnosis_logits, dim=1)


        if self.use_bpda and forced_threat_domain_indices is not None:
            dispatch_weights = F.one_hot(
                threat_domain_indices,
                num_classes=self.num_threat_domains
            ).to(diagnosis_logits.dtype)
        ...
        for layer in self.conv_block1:
            x = layer(x, threat_domain_indices, dispatch_weights=dispatch_weights, bpda=self.use_bpda, soft_forward=soft_forward)
...
class MobileViTBlock(nn.Module):
    def __init__(self, dim, depth, channel, kernel_size, patch_size, mlp_dim, dropout=0.0,
                 use_fc_conv_conv1=False, num_threat_domains=2, num_frequency_experts=2):
        super().__init__()
        self.ph, self.pw = patch_size



        self.use_fc_conv_conv1 = bool(use_fc_conv_conv1)
        self.num_threat_domains = int(num_threat_domains)
        self.num_frequency_experts = int(num_frequency_experts)
        if self.use_fc_conv_conv1:
            self.conv1_fd = FrequencyConditionalConvolution(channel, channel, kernel_size, stride=1, padding=1, groups=1, bias=False,
                                   num_frequency_experts=self.num_frequency_experts, num_threat_domains=self.num_threat_domains)
...
class TaFDMobileViT(nn.Module):
    def __init__(self, size=32, num_classes=10,
        ...
        self.mv2.append(MV2Block(channels[0], channels[1], _mv2_0_stride, expansion, use_fc_conv_dw=_use_fd_dw(0),
        ...
        _mv_kw = dict(use_fc_conv_conv1=False,
                      num_threat_domains=self.num_threat_domains, num_frequency_experts=self.num_frequency_experts)
    ...
    def forward(self, img, attack_source_ids=None, skip_normalize=False,
        ...
        for i, blk in enumerate(self.mv2):
            x = blk(x, threat_domain_indices, dispatch_weights=dispatch_weights,
                    bpda=self.use_bpda, soft_forward=soft_forward)
            if i == 4:
                x = self.mvit[0](x, threat_domain_indices, dispatch_weights=dispatch_weights,
                                 bpda=self.use_bpda, soft_forward=soft_forward)
            if i == 5:
                x = self.mvit[1](x, threat_domain_indices, dispatch_weights=dispatch_weights,
                                 bpda=self.use_bpda, soft_forward=soft_forward)
            if i == 6:
                x = self.mvit[2](x, threat_domain_indices, dispatch_weights=dispatch_weights,
                                 bpda=self.use_bpda, soft_forward=soft_forward)
...
def build_tafd_model(backbone='resnet', dataset=None, **kwargs):
    """Build the requested ablation model for the selected backbone and dataset."""
