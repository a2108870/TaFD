# -*- coding: utf-8 -*-
"""TaFD model definitions for ResNet-34 and MobileViT backbones."""
...
class MobileViTBlock(nn.Module):
    ...
    def forward(self, x, threat_domain_indices=None, dispatch_weights=None, bpda=False):
        y = x.clone()
        if self.use_fc_conv_conv1:
            x = self.conv1_fd(x, threat_domain_indices, dispatch_weights=dispatch_weights, bpda=bpda)[0]
...
class TaFDMobileViT(nn.Module):
    ...
    def forward(self, img, attack_source_ids=None, skip_normalize=False):
        ...
        for i, blk in enumerate(self.mv2):
            ...
            if i == 4:
                x = self.mvit[0](x, threat_domain_indices, dispatch_weights=dispatch_weights, bpda=self.use_bpda)
            if i == 5:
                x = self.mvit[1](x, threat_domain_indices, dispatch_weights=dispatch_weights, bpda=self.use_bpda)
            if i == 6:
                x = self.mvit[2](x, threat_domain_indices, dispatch_weights=dispatch_weights, bpda=self.use_bpda)
