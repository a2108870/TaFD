"""FC-Conv ablation without frequency decoupling.

This variant keeps diagnosis-based expert dispatch and the residual path but
feeds the original spatial features to every expert instead of decomposing
them with threat-dependent spectral masks.
"""
...
# ==================== FC-Conv Module ====================
class FrequencyConditionalConvolution(nn.Module):
    """
    Expert convolution with frequency decoupling disabled.

    The module retains the FC-Conv parameter interface for checkpoint and
    architecture compatibility, while its forward path performs expert
    convolution and diagnosis-based dispatch directly in the spatial domain.
    """
    ...
    def __init__(
            ...
            groups: int = 1,
            bias: bool = True,
            num_frequency_experts: int = 2,
            num_threat_domains: int = 2,
            freq_cache_items: int = 8
    ):
        super().__init__()

        assert num_frequency_experts >= 1, "num_frequency_experts must be >= 1"
        assert num_threat_domains == num_frequency_experts, \
            f"num_threat_domains ({num_threat_domains}) must equal num_frequency_experts ({num_frequency_experts}) for hard routing"
        ...
        self.dedicated_experts = nn.Conv2d(
            in_channels * self.num_frequency_experts,
            out_channels * self.num_frequency_experts,
            kernel_size,
            stride,
            padding,
            dilation,
            groups=self.num_frequency_experts * groups,
            bias=False
        )
        ...
        self.threat_domain_embedding = nn.Embedding(
            ...
            self.num_frequency_experts * (self.num_basis_functions + 1)
        )
    ...
    def _compute_spectral_masks(
            ...
            dispatch_weights: torch.Tensor = None,
            bpda: bool = False
    ):
        """
        ...
        use_soft = (
            ...
            and (dispatch_weights.dim() == 2)
            and (dispatch_weights.size(0) == B_batch)
            and (dispatch_weights.size(1) == self.num_threat_domains)
        )

        if use_soft:
            # soft domain embedding = w @ Embedding.weight
            W = self.threat_domain_embedding.weight.detach()              # [D, K*(M+1)]
            rw = dispatch_weights.to(W.dtype)                         # [B, D]
        ...
        params = params_flat.view(B_batch, self.num_frequency_experts, self.num_basis_functions + 1)
        bias = params[..., 0]   # a_k
        C = params[..., 1:]     # Zernike coeffs

        # scores -> softmax masks
        S_flat = torch.matmul(C, Bflat) + bias.unsqueeze(-1)  # [B, K, H*W']
        spectral_masks_flat = F.softmax(S_flat, dim=1)
        spectral_masks = spectral_masks_flat.view(B_batch, self.num_frequency_experts, H_f, W_f).unsqueeze(2)
    ...
    def _compute_unique_selected_spectral_masks(
        ...
        params = params_flat.view(num_unique, self.num_frequency_experts, self.num_basis_functions + 1)
        bias = params[..., 0]
        coeffs = params[..., 1:]

        scores = torch.matmul(coeffs, Bflat) + bias.unsqueeze(-1)
        spectral_masks = F.softmax(scores, dim=1)
    ...
    def _forward_hard_exact(self, x: torch.Tensor, threat_domain_index: torch.Tensor):
        ...
        bands_flat = x.repeat(1, self.num_frequency_experts, 1, 1)
        expert_out_flat = self.dedicated_experts(bands_flat)
        H2, W2 = expert_out_flat.shape[-2], expert_out_flat.shape[-1]
        expert_out = expert_out_flat.view(B, self.num_frequency_experts, self.out_channels, H2, W2)
        ...
        for d in torch.unique(threat_domain_index, sorted=False).tolist():
            v = torch.zeros(self.num_frequency_experts, device=x.device)
            v[d] = 1.0
    ...
    def forward(self, x, threat_domain_index=None, dispatch_weights=None, bpda: bool = False, *_, **kwargs):
        """
        ...
        Args:
            ...
            dispatch_weights: [B, K] soft weights from the threat-domain classifier (only used when bpda=True)
        ...
        use_soft = (
                ...
                and (dispatch_weights is not None)
                and (dispatch_weights.dim() == 2)
                and (dispatch_weights.size(0) == B)
                and (dispatch_weights.size(1) == self.num_frequency_experts)
        )
        ...
        w = dispatch_weights.to(expert_out.dtype).view(B, self.num_frequency_experts, 1, 1, 1)
        ...
        for d in range(self.num_threat_domains):
            ...
            if m.any():
                v = torch.zeros(self.num_frequency_experts, device=expert_out.device)
                v[d] = 1.0
                expert_usage[f'domain_{d}'] = v
