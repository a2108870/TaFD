"""FC-Conv ablation without basis-parameterized spectral masks.

This variant directly learns full-resolution spectral masks while retaining
the frequency decomposition, expert branches, and diagnosis-based dispatch.
"""
...
class FrequencyConditionalConvolution(nn.Module):
    ...
    def __init__(
        ...
        # key: "HxW", value: nn.Embedding[num_threat_domains, K * H_f * W_f]
    ...
    def _get_or_create_direct_mask_embedding(
        ...
        if key not in self.direct_mask_embeddings:
            embedding = nn.Embedding(
                ...
                self.num_frequency_experts * mask_dims,
            )
            nn.init.zeros_(embedding.weight)
    ...
    def _compute_spectral_masks(
        ...
        if use_soft:
            ...
            rw = dispatch_weights.to(weight_table.dtype)
        ...
        params = params_flat.view(B_batch, self.num_frequency_experts, mask_dims)
        spectral_masks_flat = F.softmax(params, dim=1)
    ...
    def _compute_unique_selected_spectral_masks(
        ...
        scores = params_flat.view(num_unique, self.num_frequency_experts, mask_dims)
