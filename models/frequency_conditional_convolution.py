"""Frequency-conditional convolution used by TaFD.

FC-Conv constructs threat-dependent spectral masks from a compact Zernike
basis, applies the resulting frequency decomposition, and dispatches each
sample to the expert selected by threat-domain diagnosis.
"""
...
class FrequencyConditionalConvolution(nn.Module):
    ...
    def forward(self, x, threat_domain_index=None, dispatch_weights=None, bpda: bool = False, *_, **kwargs):
        ...
        Xk = (x_fft.unsqueeze(1) * spectral_masks).reshape(B * self.num_frequency_experts, C_in, H, spectral_masks.size(-1))

        # 3) iFFT
        bands_flat = torch.fft.irfft2(Xk, s=(H, W), norm='ortho').reshape(B, self.num_frequency_experts * C_in, H, W)
