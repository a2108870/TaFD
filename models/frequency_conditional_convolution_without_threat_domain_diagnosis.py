"""FC-Conv used by the ablation without threat-domain diagnosis.

The corresponding TaFD model replaces diagnosis-based dispatch with a uniform
mixture over the frequency experts while retaining the remaining FC-Conv
computation.
"""
...
class FrequencyConditionalConvolution(nn.Module):
    ...
    def forward(self, x, threat_domain_index=None, dispatch_weights=None,
        ...
        expert_usage = {}
        if soft_forward:
            expert_usage["uniform_mix"] = dispatch_weights.detach().mean(dim=0)
        else:
            for d in range(self.num_threat_domains):
