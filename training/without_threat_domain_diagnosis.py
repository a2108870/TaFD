#!/usr/bin/env python3
"""Training implementation for the ablation without threat-domain diagnosis."""
...
from models.tafd_without_threat_domain_diagnosis import build_tafd_model
...
def prepare_gpgd_bases(args, trainloader):
    """Build or load PCA bases required by the GPGD attack."""
...
def _diagnosis_ablation_enabled(args) -> bool:
    return getattr(args, "diagnosis_ablation_mode", "none") != "none"


def _diagnosis_ablation_mode(args) -> str:
    return getattr(args, "diagnosis_ablation_mode", "none")


def _fixed_threat_domain_enabled(args) -> bool:
    return _diagnosis_ablation_mode(args) == "fixed"


def _forced_threat_domain_indices(args, batch_size: int, target_device):
    if not _fixed_threat_domain_enabled(args):
        return None
    threat_domain_index = int(getattr(args, "forced_threat_domain_index", 0))
    return torch.full((batch_size,), threat_domain_index, dtype=torch.long, device=target_device)


def _configure_diagnosis_ablation(model, args):
    threat_domain_index = int(getattr(args, "forced_threat_domain_index", 0)) if _fixed_threat_domain_enabled(args) else None
    use_uniform_mixture = _diagnosis_ablation_mode(args) == "uniform"
    if hasattr(model, "set_forced_threat_domain_index"):
        model.set_forced_threat_domain_index(threat_domain_index)
    else:
        setattr(model, "forced_threat_domain_index", threat_domain_index)
    if hasattr(model, "set_uniform_threat_domain_mixture"):
        model.set_uniform_threat_domain_mixture(use_uniform_mixture)
    else:
        setattr(model, "use_uniform_threat_domain_mixture", use_uniform_mixture)
...
def train_one_epoch(epoch, trainloader, criterion, optimizer, n_classes, model,
    ...
    _configure_diagnosis_ablation(model, args)
    ...
    for step_idx, (img, tgt) in enumerate(pbar, start=1):
        ...
        global_iter += 1
        use_uniform_expert_mixture = _diagnosis_ablation_enabled(args)
        if not use_uniform_expert_mixture:
            if epoch < 10:
        ...
        if (not use_uniform_expert_mixture) and (global_iter % args.assignment_update_interval == 0 or global_iter == 1):
            model.update_threat_domain_assignments(epoch, args.end_epoch)

        model.train()
        combined = torch.cat(imgs_mix, 0).detach()
        forced_threat_domain_indices = _forced_threat_domain_indices(args, combined.size(0), combined.device)
        if use_uniform_expert_mixture:
            target_threat_domain_indices = (
                forced_threat_domain_indices
                if forced_threat_domain_indices is not None
                else model.get_threat_domain_indices(attack_source_ids)
            )
        else:
        ...
        outputs = model(
            ...
            attack_source_ids=attack_source_ids,
            forced_threat_domain_indices=forced_threat_domain_indices
        )
        cls_logits, _, diagnosis_logits, _ = outputs

        # The diagnosis loss is disabled for this ablation.
        if use_uniform_expert_mixture:
            diagnosis_loss = diagnosis_logits.new_zeros(())
        else:
...
def validate_attack_union(epoch, testloader, criterion, model, num_classes,
    ...
    """Run validation with the paper-defined attack test set."""
    ...
    for step_idx, (img, tgt) in enumerate(pbar, start=1):
        ...
        if not _diagnosis_ablation_enabled(args):
        ...
        with torch.no_grad():
            if batch_eval_forwards and attack_inputs:
                ...
                forced_threat_domain_indices = _forced_threat_domain_indices(
                    args, combined_inputs.size(0), combined_inputs.device
                )
                out_tuple = model(
                    combined_inputs,
                    attack_source_ids=None,
                    forced_threat_domain_indices=forced_threat_domain_indices
                )
                ...
                for name, x_in in attack_inputs.items():
                    forced_threat_domain_indices = _forced_threat_domain_indices(args, x_in.size(0), x_in.device)
                    out_tuple = model(
                        x_in,
                        attack_source_ids=None,
                        forced_threat_domain_indices=forced_threat_domain_indices
    ...
    if cm.sum() > 0:
        print(f"\nThreat-domain confusion matrix (rows=true, cols=pred):")
        print(cm.cpu().numpy())
...
def main(args):
    ...
    if _diagnosis_ablation_enabled(args):
        if _fixed_threat_domain_enabled(args):
            print(f"[Ablation] threat-domain diagnosis: {args.diagnosis_ablation_mode}, threat_domain_index={args.forced_threat_domain_index}")
        else:
            print(f"[Ablation] threat-domain diagnosis: {args.diagnosis_ablation_mode}")
